...
class CreateCustomFieldCommand
{
    ...
    public function execute($headers, $listId, $type, $name, $placeHolder)
    {
        $data = array(
            'instance_in' => array(
                'data_type' => $type,
        ...
        $response = $this->apiService->call('post', '/apps/api/v1/lists/' . $listId . '/customfields', json_encode($data), $headers);
